...
- `tests/tasklist.v2.spec.ts` — UI V2 TaskList (toggle, largeurs colonnes méta ≥ 120px, prefs Actions → `localStorage`) ; auth auto via `helpers/uiAuth.ts` + `USE_LOCAL_STORE`
