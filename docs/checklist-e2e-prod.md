...
| Date | 2026-05-30 ~17h40 (run) · correctifs P0/P1 2026-05-31 (code, re-test pending) |

| Commit run | `f233fa73` |

| Décision | **GO partiel** (run) — **re-test complet requis** après deploy correctifs P0/P1 |

| Bugs ouverts | [ROADMAP.md](../ROADMAP.md) — section « Retour E2E prod 2026-05-30 » |

| Correctifs code (post-run) | P0 RGPD purge v2 + runtime ; P1 Meet datetime/PATCH ; P1 priorité compte API ; P1 Microsoft redirect UX ; P2 conflit dédup + P3 sync Archives (`4aae566`) — **re-test prod pending** |
...
3. [ ] **Modifier** l’heure → changement reflété dans Google — **NOK run 2026-05-30** ; fix livré 2026-05-31 (`toGoogleCalendarDateTime` + PATCH Meet) — **re-test requis**
...
1. [ ] Compte Google **prioritaire** = celui qui reçoit la réservation — **NOK run 2026-05-30** ; fix livré 2026-05-31 (`PUT /calendar/accounts/priority` + auto-save) — **re-test requis**
...
> **Note run 2026-05-30** : ajout compte Microsoft secondaire — OAuth ne finalise pas, redirect vers Mon profil (`/settings`). Fix livré 2026-05-31 (redirect `/agenda/manage?microsoft=error` + toasts) — **re-test requis**. Voir ROADMAP P1.
