...
function ConnectedAppsBlock({
  ...
  const apps: Array<{
    ...
    guideHref: string;
    migrateHref: string;
    migrateLabelKey: TranslationKey;
    available: boolean;
  ...
  }> = [
    {
      ...
      migrateHref: "/migrate/notion",
      migrateLabelKey: "migrate.notion.menuLabel",
      available: true,
    ...
    },
    {
      ...
      migrateHref: "/migrate/monday",
      migrateLabelKey: "migrate.monday.menuLabel",
      available: true,
              ...
              >
                ...
                <div className="mt-auto flex flex-col gap-2 pt-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {!hasIntegrations && app.available && <PlanBadge tier="small" />}
                  </div>
                  {app.available && (
                    <div className="flex w-full flex-col gap-2">
                      {!connected && (
                        <Link href={app.migrateHref} className={`${integrationActionBtn} relative z-0 block`}>
                          {t(app.migrateLabelKey)}
                        </Link>
                      )}
                          ...
                          {app.connectedActions.map((action) =>
                            action.kind === "link" ? (
                              <Link
                                key={action.href}
                                href={action.href}
                                className={`${integrationActionBtn} relative z-0 block`}
                              >
                                {t(action.labelKey)}
                              </Link>
                            ) : (
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (!app.available || !hasIntegrations) return;
                          if (connected) {
                        ...
                        className={
                          connected
                            ? "relative z-0 self-end rounded-md px-3 py-1.5 text-xs font-medium text-zinc-500 dark:text-slate-400 hover:text-zinc-800 dark:hover:text-slate-200 disabled:opacity-50 transition-colors"
                            : `${integrationConnectBtn} relative z-0 disabled:opacity-50`
                        }
                      >
...
function IntegrationsSection({ hasIntegrations }: { hasIntegrations: boolean }) {
      ...
      {!hasIntegrations ? (
        <>
          ...
          <button
            type="button"
            disabled
            className="inline-flex items-center gap-2 rounded-md border border-dashed border-zinc-300 dark:border-slate-600 px-4 py-2.5 text-sm font-medium text-zinc-600 dark:text-slate-400 w-full justify-center cursor-not-allowed"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  const hubCards: Array<{
    ...
    {
      key: "data",
      titleKey: "settings.integrationsCardData",
      descKey: "settings.integrationsCardDataDesc",
      iconBg: "bg-emerald-100 dark:bg-emerald-900/30",
      icon: (
        <svg className="w-6 h-6 text-emerald-700 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
        </svg>
      ),
      badge: t("settings.integrationsCardDataBadge"),
    },
    {
      key: "webhooks",
      ...
      {!activeSubSection ? (
        <>
          ...
          {!hasIntegrations && (
            <SoftLockHint
              tier="small"
              className="rounded-lg border border-zinc-200 dark:border-slate-700 bg-zinc-50/80 dark:bg-slate-800/40 px-3 py-2"
            />
          )}
          {loading ? (
        ...
        <>
          ...
          {activeSubSection === "data" && (
            <div className="space-y-4">
              <p className="text-sm text-zinc-500 dark:text-slate-400">{t("settings.integrationsDataHint")}</p>
              <p className="text-xs text-zinc-500 dark:text-slate-500">{t("import.templatesShortHint")}</p>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (taskImportFileRef.current) taskImportFileRef.current.value = "";
                  if (file) setTaskImportFile(file);
                }}
              />
              ...
              <TaskImportModal
                ...
                onSuccess={() => setTaskImportFile(null)}
              />
            </div>
          )}
