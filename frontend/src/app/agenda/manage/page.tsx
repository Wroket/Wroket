...
export default function ManageCalendarsPage() {
  ...
  const handleDisconnect = async (accountId: string) => {
    try {
      ...
      toast.success(t("agenda.accountDisconnected" as TranslationKey));
    } catch { /* ignore */ }
  };
  ...
  const toggleCalendar = (accountId: string, calendarId: string) => {
    setAccounts((prev) =>
      prev.map((a) =>
        a.id === accountId
          ? { ...a, cals: a.cals?.map((c) => (c.calendarId === calendarId ? { ...c, enabled: !c.enabled } : c)) }
          : a,
      ),
    );
  };
  ...
  const handleSave = async (accountId: string) => {
    ...
    try {
      ...
      toast.success(t("agenda.calendarsSaved" as TranslationKey));
    } catch { /* ignore */ }
      ...
      <div className="max-w-[700px] space-y-6">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-zinc-900 dark:text-slate-100">{t("agenda.manageCalendars" as TranslationKey)}</h1>
            <PageHelpButton
              title={t("agenda.manageCalendars" as TranslationKey)}
              items={[
                { text: t("help.manage.connect" as TranslationKey) },
                { text: t("help.manage.select" as TranslationKey) },
                { text: t("help.manage.colors" as TranslationKey) },
              ]}
            />
          </div>
          <p className="text-sm text-zinc-500 dark:text-slate-400 mt-1">{t("agenda.manageDesc" as TranslationKey)}</p>
        </div>
        ...
        >
          ...
          {accounts.length > 0 ? t("settings.addGoogleAccount" as TranslationKey) : t("agenda.connectGoogle")}
        </button>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-zinc-400 dark:text-slate-500">{t("settings.noGoogleAccounts" as TranslationKey)}</p>
          </div>
        ) : (
                ...
                <div key={account.id} className="bg-white dark:bg-slate-900 border border-zinc-200 dark:border-slate-700 rounded-lg overflow-hidden">
                  ...
                  {account.cals && account.cals.length > 0 ? (
                    <div className="px-4 py-3 space-y-1">
                      <p className="text-[10px] text-zinc-400 dark:text-slate-500 uppercase tracking-wide font-semibold mb-2">{t("settings.calendarSelect" as TranslationKey)}</p>
                      {account.cals.map((cal) => (
                        <label
                          key={cal.calendarId}
                          className="flex items-center gap-3 rounded-md px-2.5 py-2 hover:bg-zinc-50 dark:hover:bg-slate-800/50 cursor-pointer transition-colors"
                        >
                          <input
                            type="checkbox"
                            checked={cal.enabled}
                      ...
                      <div className="pt-2">
                        ...
                        >
                          {savingId === account.id ? "..." : t("projects.save" as TranslationKey)}
                        </button>
                      </div>
                    </div>
                  ) : (
                    <div className="px-4 py-4 text-center">
                      <p className="text-xs text-zinc-400 dark:text-slate-500 italic">{t("agenda.noCalendarsAvailable" as TranslationKey)}</p>
                    </div>
                  )}
                </div>
              );
