...
import { DOC_INTEGRATION_GUIDE_SLUGS, getDocGuide } from "../../_components/guideConfigs";
...
export function generateStaticParams() {
  return DOC_INTEGRATION_GUIDE_SLUGS.map((slug) => ({ slug }));
}
...
export default async function IntegrationGuidePage({ params, searchParams }: PageProps) {
  ...
  const guide = getDocGuide(slug);
  if (!guide || guide.category !== "integration") notFound();
