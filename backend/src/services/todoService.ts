...
/**
 * Returns a snapshot of the in-memory todo ids grouped by owner uid.
 * Used by the drift monitor in v2 mode where `getStore().todos` is empty
 * (hydration goes into `todosByUser`, not the legacy persistence map).
 */
export function getInMemoryTodoIdsByOwner(): Record<string, string[]> {
  const out: Record<string, string[]> = {};
  for (const [ownerUid, todos] of todosByUser) {
    out[ownerUid] = [...todos.keys()];
  }
  return out;
}
