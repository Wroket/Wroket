...
export async function getSlots(req: AuthenticatedRequest, res: Response) {
  ...
  const result = await proposeSlotsForTodo(req.user!.uid, req.user!.email ?? "", todoId);
...
export async function bookSlot(req: AuthenticatedRequest, res: Response) {
  ...
  const result = await bookTodoSlot(req.user!.uid, req.user!.email ?? "", todoId, {
    start,
    end,
    force: !!force,
  });
  ...
  res.status(200).json(todoToClientJson(result.todo));
}

export async function clearSlot(req: AuthenticatedRequest, res: Response) {
  const todoId = req.params.todoId as string;
  const updated = await clearTodoSlot(req.user!.uid, req.user!.email ?? "", todoId);
