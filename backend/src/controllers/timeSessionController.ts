...
function assertTodoAccess(req: AuthenticatedRequest, todoId: string) {
  const found = findTodoForUser(req.user!.uid, todoId);
...
export async function startTimer(req: AuthenticatedRequest, res: Response) {
  const todoId = req.params.id as string;
  assertTodoAccess(req, todoId);
  const session = startTimeTimer(req.user!.uid, req.user!.email ?? "", todoId);
...
export async function addManualSession(req: AuthenticatedRequest, res: Response) {
  ...
  const session = addManualTimeSession(req.user!.uid, todoId, durationMinutes, note, startedAt);
  res.status(201).json(session);
}
